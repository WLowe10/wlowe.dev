        ...
        <motion.div animate={{ height }} className={classes.skillCard}> 
            <div ref={measureRef} style={{position: "relative"}}>
                <AnimatePresence initial={false}>
                    {
                        !techDetails ? (
                            <motion.div style={{width: "100%"}} initial={mainVariants.exit} animate={{...mainVariants.enter, position: "static" }} exit={{...mainVariants.exit, position: "absolute" }} key={"main"}>
                                <MainTech skillGroup={skillGroup} setTech={setTech}/>
                            </motion.div>
                        ) : (
                            <motion.div style={{width: "100%"}} initial={detailsVariants.exit} animate={{...detailsVariants.enter, position: "static" }} exit={{...detailsVariants.exit, position: "absolute" }} key={"info"}>
